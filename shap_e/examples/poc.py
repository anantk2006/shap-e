...
parser.add_argument("--prompt", type = str, default="prompt.txt", help="pretty self explanatory dumbass")
...
if args.latents:
    ...
    for x in latents_noisy:
        x_T = x["x"]
        continue
    prompt = open("lastprompt.txt", "r").read() + " and " + prompt
else: 
    x_T = None
...
sys.stdout = open("lastprompt.txt", "w")
print(prompt)
